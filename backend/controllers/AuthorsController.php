...
class AuthorsController extends Controller
{
    ...
    public function actionGetAuthorsByTitle(){
        if(Yii::$app->request->isAjax){
            $query = Yii::$app->request->post('q');
            $selected_authors = Yii::$app->request->post('authors');
            $selected_authors = trim($selected_authors,',');

            $result = Authors::find()->where(['like','firstName',$query]);
            if(!empty($selected_authors)){
                $result = $result->andWhere('id NOT IN ('.$selected_authors.')');
            }
